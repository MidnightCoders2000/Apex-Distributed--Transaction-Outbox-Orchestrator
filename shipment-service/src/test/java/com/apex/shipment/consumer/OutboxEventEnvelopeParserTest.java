...
class OutboxEventEnvelopeParserTest {
    ...
    void relevantEventTypeIsRecognized() {
        ...
        assertThat(parser.isRelevant(envelope.get(), "ShipmentRequested")).isTrue();
    }
    ...
    void irrelevantEventTypeIsFiltered() {
        ...
        Optional<OutboxEnvelope> envelope = parser.parse(json);
        assertThat(envelope).isPresent();
        assertThat(parser.isRelevant(envelope.get(), "ShipmentRequested")).isFalse();
    }
    ...
    void nullOpIsNotRelevant() {
        String json = """
                {"before":null,"after":{"id":"11111111-1111-1111-1111-111111111111","event_type":"ShipmentRequested","payload":"{}"},"op":null}
                """;
        Optional<OutboxEnvelope> envelope = parser.parse(json);
        assertThat(envelope).isPresent();
        assertThat(parser.isRelevant(envelope.get(), "ShipmentRequested")).isFalse();
